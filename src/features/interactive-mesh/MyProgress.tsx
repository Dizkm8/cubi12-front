...
import Agent from "../../app/api/agent";
import { primaryGrayColor, primaryBlueColor, primaryRedColor, secondaryGreenColor, secondaryYellowColor, primaryOrangeColor } from "../../app/static/colors";
...
const subjectsState = [
  { 
    ...
    icon: <SquareIcon style={{ fontSize: "200%", color: primaryGrayColor }} /> 
  }, 
  { 
    type: "Asignaturas por cursar", 
    description: "Aquellas asignaturas que puedes cursar basado en lo que aprobaste",
    icon: <SquareIcon style={{ fontSize: "200%", color: secondaryGreenColor }} /> 
  }, 
  { 
    type: "Asignaturas fuera de proyección", 
    description: "Aquellas asignaturas que puedes cursar pero que, probablemente, no pruebas inscribir por dispersión",
    icon: <SquareIcon style={{ fontSize: "200%", color: secondaryYellowColor }} /> 
  },
  {
    type: "Asignaturas no cursadas",
    description: "Aquellas asignaturas que aun no cursas y que, probablemente, todavía no puedes cursar",
    ...
    <Box sx={{ flexGrow: 1, padding: "0 1rem 0", marginTop: "1.5rem" }}>
      <Grid container alignItems="center" justifyContent="space-between" style={{marginLeft: "9%", width: "84%" }}>
        <Typography variant="h3" component="span">
          ¡Hola{" "}
          <Typography variant="h3" component="span" style={{ color: primaryBlueColor, display: "inline" }}>
            {user.name.split(" ")[0]}
          </Typography>
          ! Bienvenido a tu progreso
        </Typography>
        <HelpIcon style={{ fontSize: "350%", color: primaryOrangeColor }} onClick={openHelpDialog} />
      </Grid>
      ...
      <Grid container alignItems="center" style={{ marginLeft: "9%", width: "84%", marginBottom: "2%" }}>
        {subjectsState.map((subjectType, index) => (
          <React.Fragment key={index}>
            {subjectType.icon}
            <Typography variant="h3" component="span" style={{ fontSize: "100%", marginRight: "2%" }}>
              {subjectType.type}
            </Typography>
          </React.Fragment>
        ))}
      ...
      <Dialog open={helpDialogOpen} onClose={closeHelpDialog}>
        <DialogTitle style={{ textAlign: "center", borderBottom: "2px solid black", fontSize: "250%" }}>Mi Progreso</DialogTitle>
        <DialogContent>
          <Typography style={{ paddingTop: "5%", paddingBottom: "2%", fontSize: "160%" }}>El significado de los colores</Typography>
          {subjectsState.map((subjectType, index) => (
            <React.Fragment key={index}>
              <Typography variant="subtitle1" style={{ marginTop: "2%", fontSize: "120%" }}>
                <Grid container alignItems="center">
                  {subjectType.icon} {subjectType.type}
                </Grid>
                <Typography style={{ marginTop: "2%", fontSize: "80%", marginLeft: "8%" }}>
                  {subjectType.description}
                </Typography>
              </Typography>
            </React.Fragment>
          ))}
          <Typography style={{ paddingTop: "2%", paddingBottom: "2%", fontSize: "160%" }}>Su Uso</Typography>
            <Grid container alignItems="center">
              <NearMeIcon style={{ fontSize: "200%", color: primaryBlueColor }} />
              <Typography style={{ marginTop: "2%", fontSize: "125%", marginLeft: "2%" }}>Seleccionar una asignatura</Typography>
            </Grid>
            <Typography style={{ marginLeft: "8%", fontSize: "100%" }}>
              Seleccionar una asignatura la marcará como aprobada. Deseleccionar una asignatura previamente aprobada la marcará como no aprobada
            </Typography>
        </DialogContent>
        <DialogActions>
